 ...
 *      Author: aitor
 *
 *  Edited by Taylor Finley on 6/13/13
 *  Description: This uses the Kinect to gather data and print the confidence of each pose
 *  		 This was originally used for testing and data gathering. Not intended to use further.
 */
...
segmentAndClassify (typename pcl::rec_3d_framework::GlobalNNPipeline<DistT, PointT, FeatureT> & global)
{
  ...
  while (camera.isActive ())
  {
    ...
    for (size_t i = 0; i < clusters.size (); i++)
    {
      ...
      //TMF - this is the float (conf) that I want to measure to 2 decimal places.  It is only showing one.  
      //TMF - global is of the GlobalNNPipeline class
	...
	//TMF  - writing values to text file
	std::ofstream outfile;
	outfile.open ("data.txt", ios::app);
	outfile << categories[kk] << "," << conf[kk] << "\n";
	outfile.close();
      ...
      //TMF keep track of reporting
      data_iter++;
      if (data_iter == max_iter){
	data_iter = 1;
	cout << "pause pose number: " << pose_count << endl;
	//TMF file stuff
	//Convert data_iter to string
	std::string file_nb = static_cast<std::ostringstream*>( &(std::ostringstream() << pose_count) )->str();
	std::string base_name = "screen_";
	std::string file_ext = ".png";
	std::string pic_name = base_name + file_nb + file_ext;
	vis.saveScreenshot(pic_name);
	pose_count++;
	sleep(20);
